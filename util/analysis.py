...
def accumulate_stats(base_dir: Path, stat_keys: List[str]) -> Dict[str, Dict[str, float]]:
    """
    ...
    for sub in os.listdir(base_dir):
        ...
        for stat in stat_keys:
            if stat not in stats:
                continue
            accum[stat][bench_name] = accum[stat].get(bench_name, 0.0) + stats[stat] * weight
...
def percent_diff(base: Optional[float], other: Optional[float]) -> Optional[float]:
